...
namespace ExPowerBox {
  ...
  event_source_t& CExBusUart::getEvtServoPosition() {
  ...
  void CExBusUart::main(void) {
    ...
    while (true) {
      ...
      if (!n) {
        ...
        chEvtSignal(parentThread_, EXBUS_TIMEOUT);
      }
      ...
      if (exDecode(rxData_)) {
        ...
        if (checkCRC() == JETI_EXBUS_PKT_CRC_OK) {
          ...
          if (exPacket_.dataId == JETI_EX_ID_CHANNEL) {
            ...
            chEvtSignal(parentThread_, EXBUS_SERVO_POSITIONS);
            continue;
          }
        ...
        chEvtSignal(parentThread_, EXBUS_CRC_ERR);
        nbExInvalidPacket_++;
        initPacket();
      }
    }
